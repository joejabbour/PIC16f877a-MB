...
unsigned char even_parity(unsigned char chbyte )
{
    unsigned char parity = 0;
    unsigned char tool = 0b00000001;
    for (int i=0 ; i<=7 ; i++){
        if (tool&&chbyte){
            parity=!parity;
        }
...
# 1 "./CRC16.h" 1
# 14 "./CRC16.h"
unsigned int CRC16 (unsigned char PDU[],char count )
{
    ...
    for (int i=0 ; i < (count-2) ;i++)
    {
        ...
        for (int j=0 ; j<8 ;j++)
        {
            if((reg_crc&0x01)==1)
            {
                reg_crc= (unsigned int) ((reg_crc>>1)^0xA001);
            }
            else
            {
                reg_crc= (unsigned int) (reg_crc>>1);
            }
...
# 27 "newmain.c" 2
# 37 "newmain.c"
const unsigned char address = 1;
unsigned char PDU[10] = {0};
unsigned char count;
unsigned char status;
unsigned char checksum [2] = {0};
unsigned int CRC = 0;
unsigned char DiscreteInputs [16] = {0};
unsigned char Coils [16] = {0};
unsigned int InputRegisters [2] = {0};
unsigned int HoldingRegisters [3] = {0};
...
void sendexception(unsigned char functioncode, unsigned char errorcode)
{
    PDU[0] = address;
    PDU[1] = functioncode | 0x80;
    PDU[2] = errorcode;
    CRC=CRC16 (PDU,3);
    PDU[3] = (unsigned char) (CRC & 0xFF);
    PDU[4] = (unsigned char) ((CRC>>8)&0xFF);
    writePDU(5);
}

unsigned char getPDU (void)
{
    unsigned char i = 0;

    unsigned char nineth=RCSTAbits.RX9D;
    PDU[i]=RCREG;
...
unsigned char processPDU (unsigned char count)
{
    if (PDU[0] != address)
        return 2;
    ...
    CRC = CRC16(PDU,count);
    checksum [1] = (unsigned char) ((CRC>>8)&0xFF);
    checksum [0] = (unsigned char) (CRC & 0xFF);
    ...
    switch (PDU[1])
    {
        case 0x01:
        {
            unsigned int reg = PDU[2]<<8 | PDU[3];
            reg--;

            unsigned int coilcount = PDU[4]<<8 | PDU[5];


            if (reg < 0 || reg > ((16 -1)-coilcount) )
            {
                sendexception(PDU[1],0x02);
                return -2;
            }
            if (coilcount > 16 || coilcount < 1)
            {
            ...
            unsigned char coilbytescount = coilcount / 8;
            unsigned char coilsleft = coilcount % 8;
            ...
            CRC=CRC16 (PDU,index);
            PDU[index++] = (unsigned char) (CRC & 0xFF);
            PDU[index++] = (unsigned char) ((CRC>>8)&0xFF);
        ...
        case 0x03:
        {
            unsigned int regadd = PDU[2]<<8 | PDU[3];
            regadd--;

            unsigned int regcount = PDU[4]<<8 | PDU[5];


            if (regadd < 0 || regadd > ((3 -1)-regcount) )
            {
                sendexception(PDU[1],0x02);
                return -2;
            }
        ...
        case 0x04:
        {
            ...
            if (regadd < 0 || regadd > ((2 -1)-regcount) )
            {
                sendexception(PDU[1],0x02);
                return -2;
            }
        ...
        case 0x05:
        {
            unsigned int regadd = PDU[2]<<8 | PDU[3];
            regadd--;
            unsigned int valueint = PDU[4]<<8 | PDU[5];
